...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponseDto acceptBooking(Long userId, Long bookingId, Boolean approved) {
        ...
        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new ValidationException("Бронирование с ID '%d' не найдено. "
    ...
    public BookingResponseDto getBookingById(Long userId, Long bookingId) {
        ...
        User booker = userRepository.findById(userId)
                .orElseThrow(() -> new ValidationException("Пользователь с ID '%d' не найден. ".formatted(userId), log));
        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new ValidationException("Бронирование с ID '%d' не найдено. "
                        .formatted(bookingId), log));
    ...
    public List<BookingResponseDto> getBookingListByOwnerId(Long ownerId, String state) {
        User owner = userRepository.findById(ownerId)
                .orElseThrow(() -> new ValidationException("Пользователь с ID '%d' не найден. ".formatted(ownerId), log));
        return switch (state) {
